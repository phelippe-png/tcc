...
begin
  ...
  if modo = 'adicionar' then
    http.Post('http://localhost:9000/producoes', stream);

  if modo = 'editar' then
  begin
    http.Request.CustomHeaders.AddValue('id', IntToStr(FidCorpoProducao));
    http.Put('http://localhost:9000/producoes', stream);
  end;

  if modo = 'excluir' then
  begin
    http.Request.CustomHeaders.AddValue('id', IntToStr(FidCorpoProducao));
    http.Delete('http://localhost:9000/producoes');
  end;
end;
...
begin
  ...
  if modo = 'editar' then
  begin
    http.Request.CustomHeaders.AddValue('id', FidProducao.ToString);
    http.Put('http://localhost:9000/producoes', stream)
  end
