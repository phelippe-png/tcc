...
type
  ...
  public
    ...
    function getByID(id: integer; tabelaBanco, campo: string): TJSONValue;
...
function TController.getByID(id: integer; tabelaBanco, campo: string): TJSONValue;
var
  I: Integer;
begin
  FDQuery.Close;
  FDQuery.SQL.Clear;
  FDQuery.SQL.Add('select * from ' + tabelaBanco + ' where ' + campo + ' = :id');
  FDQuery.ParamByName('id').AsInteger := id;
  FDQuery.Open;

  if FDQuery.RecordCount = 1 then
    Result := FDQuery.ToJSONObject
  else if FDQuery.RecordCount > 1 then
